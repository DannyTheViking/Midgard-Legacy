...
if(signupButton) signupButton.addEventListener('click', async()=>{
 ...
 const {error:playerError}=await supabaseClient.from('players').insert({id:data.user.id,username,email,gender_identity:genderIdentity,title_style:genderIdentity==='man'?'freeman':genderIdentity==='woman'?'freewoman':'freeperson',health:500,max_health:500,energy:100,max_energy:100,stamina:100,max_stamina:100,courage:100,max_courage:100,last_online:new Date().toISOString(),last_regen:new Date().toISOString()});
...
async function loadHomePage(){
 ...
 if(ticks>0){ const gain=ticks*5; for(const stat of ['health','energy','stamina','courage']) { const fallbackMax = stat === 'health' ? 500 : 100; data[stat]=Math.min(Number(data[stat] ?? 0)+gain,Number(data['max_'+stat] ?? fallbackMax)); } data.last_regen=now.toISOString(); await supabaseClient.from('players').update({health:data.health,energy:data.energy,stamina:data.stamina,courage:data.courage,last_regen:data.last_regen}).eq('id',user.id); }
