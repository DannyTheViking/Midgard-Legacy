...
async function loadHomePage(){
 ...
 set('username',data.username); set('home-username',data.username); set('level',data.level||1); set('reputation',Number(data.reputation||0).toLocaleString()); set('rank',reputationTitle(data.reputation)); set('player-rank',reputationTitle(data.reputation)); set('silver',Number(data.silver||0).toLocaleString()); set('silver-card',Number(data.silver||0).toLocaleString()); set('health',`${data.health||0} / ${data.max_health||100}`); set('energy',`${data.energy||0} / ${data.max_energy||100}`); set('stamina',`${data.stamina||0} / ${data.max_stamina||100}`); set('courage',`${data.courage||0} / ${data.max_courage||100}`);
}
