...
async function loadOakTree(){const {data:{user}}=await supabaseClient.auth.getUser();if(!user)return;const {data:p}=await supabaseClient.from('players').select('oak_unlocked').eq('id',user.id).single();if(p?.oak_unlocked&&oakButton){document.getElementById('oak-card')?.classList.remove('locked');document.getElementById('oak-image').innerHTML='🌳';oakButton.disabled=false;oakButton.innerText='🪓 Chop Oak';document.getElementById('oak-log').innerText='Ready to chop Oak.';}}
async function chopOakTree(){const {data:{user}}=await supabaseClient.auth.getUser();const {data:p}=await supabaseClient.from('players').select('energy,oak_unlocked').eq('id',user.id).single();if(!p?.oak_unlocked)return;if(p.energy<10){document.getElementById('oak-log').innerText='⚡ You need 10 energy.';return;}const item=await getItemByName(ITEM_NAMES.OAK_LOG);if(!item){document.getElementById('oak-log').innerText='❌ Run DATABASE_UPDATE.sql first.';return;}const amount=Math.floor(Math.random()*6)+3;let cart=false;try{cart=await addResourceToCartOrInventory(user.id,item.id,amount);}catch(e){document.getElementById('oak-log').innerText='❌ '+e.message;return;}if(!cart)await addInventoryById(user.id,item.id,amount);await supabaseClient.from('players').update({energy:p.energy-10,last_action:new Date().toISOString()}).eq('id',user.id);await damageEquippedTool('axe',1);await addVillageReputation(amount*2);document.getElementById('oak-log').innerHTML=`🌳 You gather <strong>${amount} Oak Logs</strong>. ${cart?'Loaded into your cart.':'Placed in inventory.'}`;loadHomePage();loadCartCard();}
