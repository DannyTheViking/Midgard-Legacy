...
begin
 ...
 update public.player_workstations
    set fuel_seconds = fuel_seconds + (v_seconds * p_quantity),
        is_running = true,
        updated_at = now()
 ...
 if not found then
   insert into public.player_workstations(player_id,station_type,fuel_seconds,is_running,updated_at)
   values(v_player,'forge',v_seconds*p_quantity,true,now());
 end if;
